...
function create_postgresdb_container() {
  ...
  docker cp backend/cre_pgsqldb.sh ${POSTGRES_CONTAINER_ID}:/tmp/cre_pgsqldb.sh
...
function build_and_run_myapp_frontend(){
  ...
  BACKEND_PORT=8000
  BACKEND_BASE_URL="http://${BACKEND_HOSTNAME}:${BACKEND_PORT}"

  echo "BACKEND_BASE_URL = ${BACKEND_BASE_URL}"
  sleep 10
  run=$(docker run --name ${MY_FRONTEND_APP_NAME} -p 4000:80 -e BACKEND_BASE_URL="http://localhost:8000" -d ${MY_FRONTEND_APP_NAME})
...
function remove_app_old_images() {
  ...
  then
    ...
    do
      echo "[INFO]: image as removed --> IMAGE_ID: $i REPOSITORY: ${IMAGE_NAME}"
      docker rmi $i
      sleep 2
    done
  else
    echo "[INFO]: image: ${IMAGE_NAME} is not exist"
    echo "[INFO]: continue .."
  fi
  sleep 5
}
...
stop_and_remove_container ${MY_FRONTEND_APP_NAME}
...
# remove frontend old images
remove_app_old_images ${MY_FRONTEND_APP_NAME}

# remove tasker old images
remove_app_old_images ${MY_BACKEND_APP_NAME}
